 ...
 */
    ...
    function readString(walker) {
        ...
        walkLoop:
        while ((charCode = walker.nextCode())) {
            switch (charCode) {
                case 92: // \
                    walker.go(1);
                    break;
    ...
    each(
        ['set', 'remove', 'unshift', 'shift', 'push', 'pop', 'splice'], function (method) {
    ...
    function ForNode(aNode, owner, scope, parent, reverseWalker) {
        ...
        if (reverseWalker) {
            var me = this;
            each(
                evalExpr(this.param.value, this.scope, this.owner), function (item, i) {
